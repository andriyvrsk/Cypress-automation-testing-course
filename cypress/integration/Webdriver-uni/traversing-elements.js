...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("children() to get the children of DOM elements", () => {
    cy.get('.traversal-breadcrumb').children('.active').should('contain', 'Contact Us');                                                    //question here
  });
  ...
  it("filter() to retrieve DOM elements that match a specific selector", () => {
    cy.get('.btn-group-toggle > *').filter('.active').should('contain', 'Button-1'); //locator question
  });
  ...
  it("nextUntil() to get all of the next sibling DOM elements within elements until another element", () => {
    cy.get('#coffee').contains('Coffee').nextUntil('#milk').should('contain', 'Tea');
  });
